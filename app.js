...
function normalize(arr) { return [...arr].sort().join(''); }
function getUserAnswer(qid) { return [...document.querySelectorAll(`[name="q-${qid}"]:checked`)].map(el => el.value); }
...
function renderQuiz() {
  ...
  quizForm.querySelectorAll('input').forEach(input => input.addEventListener('change', updateProgress));
  updateProgress();
}
